...
if ($cyear) {
    ...
    $query = "SELECT * FROM department_data WHERE cyear = ? AND staff_id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ss", $cyear, $sid); // Bind the variables
    $stmt->execute();
    ...
    <nav class="navbar">
        <div class="nav-content">
            ...
            <div class="nav-links">
                ...
                <a href="stahome.php" class="nav-link active">
                ...
                <tr style="border-bottom: 1px solid #dee2e6; transition: background-color 0.3s ease;">
                    ...
                    <td style="padding: 12px 15px;">
                        <?php if (empty($row['mat'])): ?>
                            <a href="upload_mat.php?subid=<?php echo $row['subject_code']; ?>&cyear=<?php echo $cyear; ?>" style="display: inline-block; padding: 6px 12px; background-color: #007bff; color: white; text-decoration: none; border-radius: 4px; font-size: 14px; transition: background-color 0.3s ease;">Upload Mat</a>
                        <?php else: ?>
                            <span style="display: inline-block; padding: 6px 12px; background-color: #28a745; color: white; border-radius: 4px; font-size: 14px;">Uploaded Successfully</span>
                        <?php endif; ?>
                    </td>
                    <td style="padding: 12px 15px;">
                        <?php if (empty($row['que'])): ?>
                            <a href="upload_que.php?subid=<?php echo $row['subject_code']; ?>&cyear=<?php echo $cyear; ?>" style="display: inline-block; padding: 6px 12px; background-color: #007bff; color: white; text-decoration: none; border-radius: 4px; font-size: 14px; transition: background-color 0.3s ease;">Upload Que</a>
                        <?php else: ?>
                            <span style="display: inline-block; padding: 6px 12px; background-color: #28a745; color: white; border-radius: 4px; font-size: 14px;">Uploaded Successfully</span>
                        <?php endif; ?>
                    </td>
